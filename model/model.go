...
func (m *Model) InputObjects() []InputObject {
	objs := []InputObject{}
	for _, def := range m.Doc.Definitions {
		def, ok := def.(*ast.InputObjectDefinition)
		if ok {
			obj := InputObject{Def: def, Model: m}
			objs = append(objs, obj)
		}
	}
	return objs
...
// CustomInputObjects returns only the input objects defined by the user in .graphql files.
func (m *Model) CustomInputObjects() []InputObject {
	objs := []InputObject{}
	for _, obj := range m.InputObjects() {
		if obj.Def.Loc != nil {
			objs = append(objs, obj)
		}
	}
...
// GeneratedInputObjects returns only the input objects generated by Dolphin (e.g., WhereInput, CreateInput).
func (m *Model) GeneratedInputObjects() []InputObject {
	objs := []InputObject{}
	for _, obj := range m.InputObjects() {
		if obj.Def.Loc == nil {
			objs = append(objs, obj)
...
func (m *Model) HasInputObject(name string) bool {
	for _, obj := range m.InputObjects() {
		if obj.Def.Name.Value == name {
			return true
		}
	}
	return false
...
func (m *Model) InputObject(name string) InputObject {
	for _, obj := range m.InputObjects() {
		if obj.Def.Name.Value == name {
			return obj
		}
	}
	log.Fatalf("InputObject with name %s not found in model", name)
	return InputObject{}
}
