...
func (qf *UserQueryFilter) Apply(ctx context.Context, db *gorm.DB, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	...
	if *qf.Query != "" {
		...
		for _, part := range queryParts {
			ors := []string{}
			if err := qf.applyQueryWithFields(db, fields, part, TableName("users", ctx), &ors, values, joins); err != nil {
				return err
			}
...
func (qf *UserQueryFilter) applyQueryWithFields(db *gorm.DB, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	...
	// if fs, ok := fieldsMap["userRoles"]; ok {
	// 	_fields := []*ast.Field{}
	...
	// 		}
	// 	}
	// 	q := UserRoleQueryFilter{qf.Query}
	// 	err := q.applyQueryWithFields(db, _fields, query, _alias, ors, values, joins)
	// 	if err != nil {
	// 		return err
	// 	}
...
func (qf *ProfileQueryFilter) Apply(ctx context.Context, db *gorm.DB, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	if qf.Query == nil {
		return nil
	}
	...
	if *qf.Query != "" {
		...
		for _, part := range queryParts {
			ors := []string{}
			if err := qf.applyQueryWithFields(db, fields, part, TableName("profiles", ctx), &ors, values, joins); err != nil {
				return err
			}
			*wheres = append(*wheres, "("+strings.Join(ors, " OR ")+")")
		}
...
func (qf *ProfileQueryFilter) applyQueryWithFields(db *gorm.DB, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	if len(fields) == 0 {
		return nil
	}
	...
	if _, ok := fieldsMap["avatar"]; ok {

		column := alias + "." + SnakeString("avatar")

		*ors = append(*ors, fmt.Sprintf("%[1]s LIKE ? OR %[1]s LIKE ?", column))
		*values = append(*values, query+"%", "%"+query+"%")
	}

	if _, ok := fieldsMap["bio"]; ok {

		column := alias + "." + SnakeString("bio")
...
func (qf *TaskQueryFilter) Apply(ctx context.Context, db *gorm.DB, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	...
	if *qf.Query != "" {
		...
		for _, part := range queryParts {
			ors := []string{}
			if err := qf.applyQueryWithFields(db, fields, part, TableName("tasks", ctx), &ors, values, joins); err != nil {
				return err
			}
			*wheres = append(*wheres, "("+strings.Join(ors, " OR ")+")")
		}
...
func (qf *TaskQueryFilter) applyQueryWithFields(db *gorm.DB, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	...
	if _, ok := fieldsMap["priority"]; ok {

		cast := "TEXT"
		if db.Name() == "mysql" {
			cast = "CHAR"
		}
		column := fmt.Sprintf("CAST(%s"+SnakeString("priority")+" AS %s)", alias+".", cast)
...
type UserRoleQueryFilter struct {
	Query *string
}

func (qf *UserRoleQueryFilter) Apply(ctx context.Context, db *gorm.DB, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	if qf.Query == nil {
		return nil
	}
	...
	if *qf.Query != "" {
		...
		for _, part := range queryParts {
			ors := []string{}
			if err := qf.applyQueryWithFields(db, fields, part, TableName("user_roles", ctx), &ors, values, joins); err != nil {
				return err
			}
			*wheres = append(*wheres, "("+strings.Join(ors, " OR ")+")")
		}
...
func (qf *UserRoleQueryFilter) applyQueryWithFields(db *gorm.DB, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	if len(fields) == 0 {
		return nil
	}
	...
	if _, ok := fieldsMap["permissions"]; ok {

		column := alias + "." + SnakeString("permissions")
...
func (qf *TagQueryFilter) Apply(ctx context.Context, db *gorm.DB, selectionSet *ast.SelectionSet, wheres *[]string, values *[]interface{}, joins *[]string) error {
	if qf.Query == nil {
		return nil
	}
	...
	if *qf.Query != "" {
		...
		for _, part := range queryParts {
			ors := []string{}
			if err := qf.applyQueryWithFields(db, fields, part, TableName("tags", ctx), &ors, values, joins); err != nil {
				return err
			}
			*wheres = append(*wheres, "("+strings.Join(ors, " OR ")+")")
		}
...
func (qf *TagQueryFilter) applyQueryWithFields(db *gorm.DB, fields []*ast.Field, query, alias string, ors *[]string, values *[]interface{}, joins *[]string) error {
	if len(fields) == 0 {
		return nil
	}
	...
	if _, ok := fieldsMap["color"]; ok {

		column := alias + "." + SnakeString("color")
